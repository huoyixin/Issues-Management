...
<<<<<<< HEAD
  function selectTag(){
        console.log(MyTag);
        var selectTags = query('.tagMenu');
        array.forEach(selectTags, function (addtag) {
            //get issueId
            var meta = addtag.parentNode.previousSibling;
            meta = getPreElement(meta);
            var spanNode = query('.issue-meta', meta);
            spanNode = getNextElement(spanNode[0].firstChild);
            var texts = spanNode.firstChild.nodeValue;
            var issueId = texts.match(/\d+/);
            //get own tags
            var spans = query('span.badge.usertag' + issueId, addtag.parentNode);
            var selectItems = array.map(spans, function (span) {
                return span.firstChild.nodeValue;
            })
            //declare addtagPanel
            var addtagPanel = new menuPanel({
                label: '<span class="octicon octicon-plus"></span>',
                box: dom.byId('Container'),
                multipleChoice: true,
                selectedItems: selectItems
            });
            addtagPanel.placeAt(addtag).startup();
            addtagPanel.setItems(MyTag);
            //get tagBox
            var tagBox = query('.tagBox.box' + issueId)[0];
            //set event of addtagPanel
            addtagPanel.on('onMenuClick', lang.hitch(this, function (item) {
                ...
                //get alltags of tagBox and add class in it
    ...
    function getPreElement (ele) {
        if (ele.nodeType == 1) {
            ...
            return getPreElement(ele.previousSibling);
=======
  function selectTag() {
...
>>>>>>> 15a635176475e0a972669997a946bf13a11213cd
        }
